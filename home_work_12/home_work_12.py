...
class AddressBook(UserDict):
    ...
    def __next__(self):
        if self.POS < len(self.data.keys()):
            cont = ""
            i = self.POS
    ...
    def save_book(self, filename: str = "database.txt"):
        try:
    ...
    def load_book(self, filename: str = "database.txt"):
        try:
...
def handler(user_msg):
    ...
    if list_msg[0] == "hello":
        ...
        return (OPERATIONS["search"](list_msg[1]))
    else:
